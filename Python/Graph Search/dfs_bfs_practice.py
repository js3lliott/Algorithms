...
def breadth_first_search(g, start_node, stop_node):
    """
    Given a graph g, and the identifiers for two of the nodes in the graph, star_node
    and stop_node, using BFS determine if there is a path between the two nodes. If there
    is a path return a list containing the names of the nodes that comprise the path
    in the order that they wull be reached from start to finish.
...
def breadth_first_search_by_criteria(g, start_node, stop_criteria):
    ...
    and a function stop_criteria, using BFS determine if there is a path between the start_node
    and any node matching the stop_criteria. Nodes match the criteria if stop_criteria(node) == True

    If there is a path return a list containing the names of the nodes that comprise the
    path in the order that they will be reached from start to finish.

    If there is no path, return None

    if the start_node matches the stop_criteria, return [start_node]
    """
    frontier = deque() # We use appendleft and pop so this behaves as a queue
    frontier.appendleft(QueueItem(start_node, None, 0))
...
def _get_path_from_explored(explored_list, start_node, stop_node):
    """
    Simple helper function that walks backwards from the stop_node to the start_node given
