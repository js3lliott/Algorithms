...
# Named tuple instead of class, for brevity
...
class UndirectedGraph:
    ...
    def add_node(self, name):
        ...
        Return value indicates if a node was actually added, True means a new node was created and
        added. False means that anode with that name already exists in this graph, and it was not
        overwritten.
        """
    ...
    def add_edge(self, a_name, b_name, weight=1):
        ...
        if (self._adjacency_table[a_name].get(b_name) != None or self._adjacency_table[b_name].get(a_name)):
            return False
    ...
    def nodes(self):
        """
        Return an iterator that yields the nodes name (in no particular order).
        """
